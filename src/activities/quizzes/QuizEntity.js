...
export class QuizEntity extends Entity {
	...
	canEditName() {
		return (
			this._entity &&
			this._entity.hasActionByName(Actions.quizzes.updateName)
		);
	}
	...
	async setName(name) {
		const action =
			this.canEditName() &&
			this._entity.getActionByName(Actions.quizzes.updateName);
		if (!action) {
			return;
		}
	 ...
	 */
	canEditPassword() {
		const passwordEntity = this._entity.getSubEntityByRel(
			Rels.Quizzes.password
		);
		return (
			passwordEntity &&
			passwordEntity.hasActionByName(Actions.quizzes.updatePassword)
		);
	}
	...
	canEditHints() {
		...
		return (
			hintsEntity &&
			hintsEntity.hasActionByName(Actions.quizzes.updateHints)
		);
	}
	...
	canEditDisableRightClick() {
		const entity = this._entity.getSubEntityByRel(
			Rels.Quizzes.disableRightClick
		);
		return (
			entity &&
			entity.hasActionByName(Actions.quizzes.updateDisableRightClick)
		);
	}
	...
	canEditDisablePagerAndAlerts() {
		const entity = this._entity.getSubEntityByRel(
			Rels.Quizzes.disablePagerAndAlerts
		);
		return (
			entity &&
			entity.hasActionByName(Actions.quizzes.updateDisablePagerAndAlerts)
		);
	}
	...
	canEditNotificationEmail() {
		const entity = this._entity.getSubEntityByRel(
			Rels.Quizzes.notificationEmail
		);
		return (
			entity &&
			entity.hasActionByName(Actions.quizzes.updateNotificationEmail)
		);
	}
	...
	canEditPreventMovingBackwards() {
		const entity = this._entity.getSubEntityByRel(
			Rels.Quizzes.preventMovingBackwards
		);
		return (
			entity &&
			entity.hasActionByName(Actions.quizzes.updatePreventMovingBackwards)
		);
	}
	...
	canEditAutoSetGraded() {
		const entity = this._entity.getSubEntityByRel(
			Rels.Quizzes.autoSetGraded
		);
		return (
			entity &&
			entity.hasActionByName(Actions.quizzes.updateAutoSetGraded)
		);
	}
	...
	async save(quiz) {
		if (!quiz) return;
		const updateNameAction = this.canEditName()
			? this._formatUpdateNameAction(quiz)
			: null;
		const updateShuffleAction = this.canEditShuffle()
			? this._formatShuffleAction(quiz)
			: null;
		const updateHintsAction = this.canEditHints()
			? this._formatUpdateHintsAction(quiz)
			: null;
		const updateDisableRightClickAction = this.canEditDisableRightClick()
			? this._formatUpdateDisableRightClickAction(quiz)
			: null;
		const updateDisablePagerAndAlerts = this.canEditDisablePagerAndAlerts()
			? this._formatUpdateDisablePagerAndAlerts(quiz)
			: null;
		const updatePasswordAction = this.canEditPassword()
			? this._formatUpdatePasswordAction(quiz)
			: null;
		const updateNotificationEmail = this.canEditNotificationEmail()
			? this._formatNotificationEmailAction(quiz)
			: null;
		const updatePreventMovingBackwards = this.canEditPreventMovingBackwards()
			? this._formatUpdatePreventMovingBackwards(quiz)
			: null;
		const updateAutoSetGradedAction = this.canEditAutoSetGraded()
			? this._formatUpdateAutoSetGraded(quiz)
			: null;
	...
	_generateHintsAction(allowHints) {
		...
		const fields = [{ name: 'allowHints', value: allowHints }];
	...
	_formatShuffleAction(quiz) {
		...
		const fields = [{ name: 'shuffle', value: quiz.shuffle }];
	...
	_formatUpdateDisableRightClickAction(quiz) {
		...
		const action = entity.getActionByName(
			Actions.quizzes.updateDisableRightClick
		);
		if (!action) return;
	...
	_formatUpdateDisablePagerAndAlerts(quiz) {
		if (!quiz) return;
		if (!this._hasDisablePagerAndAlertsChanged(quiz.disablePagerAndAlerts))
			return;
		...
		const action = entity.getActionByName(
			Actions.quizzes.updateDisablePagerAndAlerts
		);
		if (!action) return;

		const fields = [
			{
				name: 'disablePagerAndAlerts',
				value: quiz.disablePagerAndAlerts,
			},
		];
	...
	_formatUpdatePasswordAction(quiz) {
		...
		const fields = [{ name: 'password', value: quiz.password }];
	...
	_formatNotificationEmailAction(quiz) {
		...
		const action = entity.getActionByName(
			Actions.quizzes.updateNotificationEmail
		);
		if (!action) return;
	...
	_formatUpdatePreventMovingBackwards(quiz) {
		if (!quiz) return;
		if (
			!this._hasPreventMovingBackwardsChanged(quiz.preventMovingBackwards)
		)
			return;
		...
		const action = entity.getActionByName(
			Actions.quizzes.updatePreventMovingBackwards
		);
		if (!action) return;
		...
		const fields = [
			{
				...
				value: quiz.preventMovingBackwards,
			},
		];
	...
	_formatUpdateAutoSetGraded(quiz) {
		...
		const action = entity.getActionByName(
			Actions.quizzes.updateAutoSetGraded
		);
		if (!action) return;

		const fields = [{ name: 'autoSetGraded', value: quiz.autoSetGraded }];
	...
	equals(quiz) {
		const diffs = [
			...
			[
				this.isPreventMovingBackwardsEnabled(),
				quiz.preventMovingBackwards,
			],
	...
	async delete() {
		const action =
			this.canDelete() &&
			this._entity.getActionByName(Actions.quizzes.delete);
		if (!action) {
			return;
		}
