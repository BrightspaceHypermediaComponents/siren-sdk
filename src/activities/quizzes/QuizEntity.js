...
export class QuizEntity extends Entity {
	...
	*/
	...
	attemptsAllowed() {
		const entity = this._entity.getSubEntityByRel(Rels.Quizzes.attempts);
		if (entity && entity.properties && entity.properties.attempts) {
			return entity.properties.attempts;
		}
	...
	canUpdateAttemptsAllowed() {
		const entity = this._entity.getSubEntityByRel(Rels.Quizzes.attempts);
		return entity.hasActionByName(Actions.quizzes.updateAttemptsAllowed);
	}
	...
	_generateAttemptsAllowedAction(attemptsAllowed) {
		...
		const attemptsEntity = this._entity.getSubEntityByRel(Rels.Quizzes.attempts);

		if (attemptsEntity) {
			action = attemptsEntity.getActionByName(Actions.quizzes.updateAttemptsAllowed);
		}
